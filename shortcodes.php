...
function ccta_image_box_button_link2( $atts, $content = null ) {
 ...
 if (is_array($atts)) {
    ...
    $html = '
	<div class="cta-img-con-2 text-center col-md-12 " itemscope itemtype="http://schema.org/Offer">
		<div class="col-md-5 ctab ctabthumbnail">
			<img itemprop="image" class="ctab-image " src="'.$imagesrc.'" alt="'.$alt.'" onclick="window.open(\''.$link.'\');" data-qazy="true" >
		</div>		
		<div class=" col-md-7">
			<h4 itemprop="name" class="ctab-text-'.$titlecolor.'">'.$headline.'</h4>
			<p itemprop="description" class="BoxTextContent ">' . do_shortcode($content) . '</p>
			<span  itemprop="url" class="cta-link ctabtn ctabtn-'.$color.'  ctabtn-'.$btnsize.'" onclick="window.open(\''.$link.'\');"  >'.$btntext.'</span>
		</div>
